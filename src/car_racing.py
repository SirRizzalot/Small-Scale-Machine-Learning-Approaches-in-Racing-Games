    ...
    while not quit:
        ...
        while True:
            ...
            if steps % 100 == 0 or terminated or truncated:
                ...
                print(s.shape)
                states = np.append(states, np.array([s]), axis = 0)
                actions = np.append(actions, np.array([a]), axis = 0)
            steps += 1
            if terminated or truncated or restart or quit:
                break

    with open('states_data.npy', 'wb') as file1:
        with open('actions_data.npy', 'wb') as file2:
            np.save(file1, states)
            np.save(file2, actions)
